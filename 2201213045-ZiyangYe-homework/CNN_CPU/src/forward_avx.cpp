...
// 							__m128 filterItem = _mm_broadcast_ss(ppw++); 
// 							__m128 imageItem = _mm_loadu_ps(ppi + wy * width_image_input_CNN + wx);
// 						// 	sum +=  * ppi[wy * width_image_input_CNN + wx];
// 							sum = _mm_fmadd_ps(filterItem, imageItem, sum);
// 						}
// 					}
// 					// neuron_C1[index] += sum;
// 					_mm_storeu_ps(neuron_C1 + index, sum);
// 				}
...
// 					for (int wy = 0; wy < height_kernel_conv_CNN; wy++) {
// 						for (int wx = 0; wx < width_kernel_conv_CNN; wx++) {
//                             __m256 filterItem = _mm256_broadcast_ss(ppw++); 
// 							__m256 imageItem = _mm256_loadu_ps(ppi + wy * width_image_input_CNN + wx);
//                             sum = _mm256_fmadd_ps(filterItem, imageItem, sum);
// 							// sum += *ppw++ * ppi[wy * width_image_S2_CNN + wx];
// 						}
// 					}
// 					// neuron_C3[index] += sum;
//                     _mm256_storeu_ps(neuron_C3 + index, sum);
// 				}
// 				neuron_C3[index] += bias_C3[channel];     //加偏置
// 				neuron_C3[index] = activation_function_tanh(neuron_C3[index]);  //激励函数
// 			for (int x = 8; x < width_image_C3_CNN; x++) {
// 				int index = (channel*height_image_C3_CNN*width_image_C3_CNN) + y*width_image_C3_CNN + x;  //当前神经元
// 				neuron_C3[index] = 0.0;
// 				//卷积运算
// 				for (int inc = 0; inc < num_map_S2_CNN; inc++) {
